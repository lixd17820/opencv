...
public class CharsSegment {
	...
	public int charsSegment(final Mat input, Vector<Mat> resultVec) {
		...
		Mat img_threshold = new Mat(input.rows(), input.cols(), CvType.CV_8UC1);

		Mat input_grey = new Mat();
		Imgproc.cvtColor(input, input_grey, Imgproc.COLOR_RGB2GRAY);

		int w = input.cols();
		int h = input.rows();
		Mat tmpMat = new Mat(input, new Rect((int) (w * 0.1), (int) (h * 0.1),
				(int) (w * 0.8), (int) (h * 0.8)));
		Color color = CoreFunc.getPlateType(tmpMat, true);

		if (color == Color.BLUE) {
			System.out.println("这是一个蓝牌");
			Imgproc.threshold(input_grey, img_threshold, 10, 255,
					Imgproc.THRESH_OTSU + Imgproc.THRESH_BINARY);
		} else if (color == Color.YELLOW) {
			System.out.println("这是一个黄牌");
			Imgproc.threshold(input_grey, img_threshold, 50, 255,
		...
		if (this.isDebug) {
			Imgcodecs.imwrite("tmp/debug_input_grey.jpg", input_grey);
			Imgcodecs.imwrite("tmp/debug_char_threshold.jpg", img_threshold);
		}
		...
		if (this.isDebug) {
			...
			Imgcodecs.imwrite(str, img_threshold);
		}
		...
		Imgproc.findContours(img_contours, contours, img_contours, // a vector
																	// of
																	// contours
		...
		if (this.isDebug) {
			...
			Imgcodecs.imwrite(str, chineseMat);
		}
	...
	private Mat preprocessChar(Mat in) {
		...
		Imgproc.warpAffine(in, warpImage, transformMat, warpImage.size(),
	...
	public static void main(String[] args) {
		Mat in = Imgcodecs
				.imread("D:\\old_project\\EasyPR-Java\\res\\image\\test_image\\core_func_yellow.jpg");
		Vector<Mat> v = new Vector<Mat>();
		CharsSegment seg = new CharsSegment();
		seg.isDebug = true;
		int r = seg.charsSegment(in, v);
